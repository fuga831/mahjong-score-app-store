...
require("../www/store");
describe('Utils.md5toUUID', function () {
    test('converts MD5 hash to UUIDv3-like format', function () {
        // MD5 of "hello" is 5d41402abc4b2a76b9719d911017c592
        // UUIDv3 format: xxxxxxxx-xxxx-3xxx-8xxx-xxxxxxxxxxxx
        // Position 12: version '3', position 16: variant '8'
        var result = CdvPurchase.Utils.md5toUUID('hello');
        expect(result).toBe('5d41402a-bc4b-3a76-8971-9d911017c592');
    });
    test('produces valid UUID format', function () {
        var result = CdvPurchase.Utils.md5toUUID('test');
        expect(result).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
    });
    test('version nibble is 3 (UUIDv3)', function () {
        var result = CdvPurchase.Utils.md5toUUID('hello');
        expect(result === null || result === void 0 ? void 0 : result.charAt(14)).toBe('3');
    });
    test('variant nibble is 8 (RFC 4122)', function () {
        var result = CdvPurchase.Utils.md5toUUID('hello');
        expect(result === null || result === void 0 ? void 0 : result.charAt(19)).toBe('8');
    });
    test('is deterministic', function () {
        expect(CdvPurchase.Utils.md5toUUID('user1')).toBe(CdvPurchase.Utils.md5toUUID('user1'));
    });
    test('different inputs produce different outputs', function () {
    ...
    test('preserves remaining MD5 hex in UUID', function () {
        // MD5 of "a" is 0cc175b9c0f1b6a831c399e269772661
        var result = CdvPurchase.Utils.md5toUUID('a');
        var md5 = CdvPurchase.Utils.md5('a');
